...
if ($num_images > 0) {
    ...
    if (ZCA_PHOTOSWIPE_STATUS == 'true') {         
        $height = '1024';
        $width = '1024';
            $script_link = '<a href="' . zca_photoswipe($products_image_large, '', MEDIUM_IMAGE_WIDTH, MEDIUM_IMAGE_HEIGHT) . '" itemprop="contentUrl" data-size="'.$width.'x'.$height.'" class="mx-auto d-block img-fluid"><img src="' . zca_photoswipe($base_image, $products_name, SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '" itemprop="thumbnail" class="mx-auto d-block img-fluid" /></a><figcaption itemprop="caption description">' . stripslashes($products_name) . '</figcaption></figure>';
    } else {        
...
//-eof-zca_diy_tpl_demo  *** 1 of 0 ***       
    ...
    if (ZCA_PHOTOSWIPE_STATUS == 'true') {    
        $list_box_contents[$row][$col] = array('params' => '',
            'text' => '<figure itemprop="associatedMedia" itemscope itemtype="http://schema.org/ImageObject" class="columns'.$col_per_row.'  content-align-center">' . $script_link . '');
    } else {
