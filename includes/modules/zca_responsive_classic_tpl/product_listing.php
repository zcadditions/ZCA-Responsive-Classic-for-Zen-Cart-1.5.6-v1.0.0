...
$col_per_row = PRODUCT_LISTING_COLUMNS_PER_ROW;
} else if ( $detect->isMobile() && !$detect->isTablet() || $_SESSION['layoutType'] == 'mobile' ) {
$col_per_row = PRODUCT_LISTING_COLUMNS_PER_ROW_MOBILE;
} else if ( $detect->isTablet() || $_SESSION['layoutType'] == 'tablet'){
$col_per_row = PRODUCT_LISTING_COLUMNS_PER_ROW_TABLET;
} else {
$col_per_row = PRODUCT_LISTING_COLUMNS_PER_ROW;
}
...
if(isset($_SESSION['PRODUCT_LISTING_LAYOUT_STYLE']) and  ($_SESSION['PRODUCT_LISTING_LAYOUT_STYLE'] == 'rows' or $_SESSION['PRODUCT_LISTING_LAYOUT_STYLE'] == 'columns') ) {
...
//zca
  ...
  if ($listing_layout_style == 'columns') {
    if ($num_products_count < $col_per_row || $col_per_row == 0 ) {
      ...
      $col_width = floor(100/$col_per_row) - 0.5;
    }
  }
//zca   
    ...
    for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
      ...
      switch ($column_list[$col]) {
        ...
        if (zen_has_product_attributes($listing->fields['products_id']) or PRODUCT_LIST_PRICE_BUY_NOW == '0') {
          $lc_button = '<a href="' . zen_href_link(zen_get_info_page($listing->fields['products_id']), 'cPath=' . (($_GET['manufacturers_id'] > 0 and $_GET['filter_id']) > 0 ?  zen_get_generated_category_path_rev($_GET['filter_id']) : ($_GET['cPath'] > 0 ? $_GET['cPath'] : zen_get_generated_category_path_rev($listing->fields['master_categories_id']))) . '&products_id=' . $listing->fields['products_id']) . '" role="button">' . MORE_INFO_TEXT . '</a>';
        } else {
...
//zca
    ...
    if ($listing_layout_style == 'columns') {
      ...
      $list_box_contents[$rows][$column] = array('params' =>'class="centerBoxContentsListing centeredContent columns' . $col_per_row . '"', 
